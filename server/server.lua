...
for k, v in pairs(Config.Consumables) do
	QBCore.Functions.CreateUseableItem(k, function(source, item) TriggerClientEvent('qb-consumables:Consume', source, item.name) end)
...
RegisterNetEvent("qb-consumables:server:DupeWarn", function(item, newsrc)
...
RegisterNetEvent('qb-consumables:server:toggleItem', function(give, item, amount, newsrc)
	local src = newsrc or source
	...
	if not give then
		...
		else TriggerEvent("qb-consumables:server:DupeWarn", item, src) end -- if not boot the player
	elseif give then
...
RegisterNetEvent('qb-consumables:server:addNeed', function(amount, type)
...
QBCore.Functions.CreateCallback('qb-consumables:server:syncConsumables', function(source, cb) cb(Consumables) end)
QBCore.Functions.CreateCallback('qb-consumables:server:syncEmotes', function(source, cb) cb(Emotes) end)

RegisterNetEvent('qb-consumables:server:syncAddItem', function(itemName, data)
	if not Consumables[itemName] then
		QBCore.Functions.CreateUseableItem(itemName, function(source, item) TriggerClientEvent('qb-consumables:Consume', source, itemName) end)
		if Config.Debug then print("^5Debug^7: ^2Adding new ^3Item^7: '^6"..itemName.."^7'") end
		Consumables[itemName] = data
		TriggerClientEvent("qb-consumables:client:syncConsumables", -1, Consumables)
	end
end)

RegisterNetEvent('qb-consumables:server:syncAddEmote', function(emoteName, data)
	if not Emotes[emoteName] then
		...
		TriggerClientEvent("qb-consumables:client:syncEmotes", -1, Emotes)
	end
end)

RegisterNetEvent("qb-consumables:server:syncConsumables", function() TriggerClientEvent('qb-consumables:client:syncConsumables', -1, Consumables) end)
RegisterNetEvent("qb-consumables:server:syncEmotes", function() TriggerClientEvent('qb-consumables:client:syncEmotes', -1, Emotes) end)
