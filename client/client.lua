...
local function syncConsumables()
	local p = promise.new() QBCore.Functions.TriggerCallback('qb-consumables:server:syncConsumables', function(cb) p:resolve(cb) end) Consumables = Citizen.Await(p)
	local p2 = promise.new() QBCore.Functions.TriggerCallback('qb-consumables:server:syncEmotes', function(cb) p2:resolve(cb) end) Emotes = Citizen.Await(p2)
    if Config.Debug then print("^5Debug^7: ^2Retrieved ^6"..countTable(Consumables).." ^2Items and ^6"..countTable(Emotes).." ^2Emotes^7") end
    propConvert()
end

RegisterNetEvent("qb-consumables:client:syncConsumables", function(NewConsumables)
...
RegisterNetEvent("qb-consumables:client:syncEmotes", function(NewEmotes)
...
RegisterNetEvent('qb-consumables:Consume', function(itemName)
	...
	if not cancelled then
        ...
        if stats then
            if stats.hunger then TriggerServerEvent("qb-consumables:server:addNeed", QBCore.Functions.GetPlayerData().metadata["hunger"] + stats.hunger, "hunger") end
            if stats.thirst then TriggerServerEvent("qb-consumables:server:addNeed", QBCore.Functions.GetPlayerData().metadata["thirst"] + stats.thirst, "thirst") end
        end
